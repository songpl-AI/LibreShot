...
> 状态：历史设计方案，覆盖 1 个 bug + 4 个许愿功能。以下内容保留当时的方案讨论，不作为当前实现说明。
>
> 2026-09-13 更新：现行行为以 [README](../../README.md) 和 [1.1.1 更新说明](../releases/1.1.1.md) 为准。Esc 已改为一次取消截图；文字采用内联编辑器；翻译入口要求 macOS 26+，缺少语言包时使用系统下载弹窗。工具栏可隐藏和排序。
